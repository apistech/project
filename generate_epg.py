...
def load_base_epg() -> ET.Element:
    if os.path.exists(OUTPUT_XML):
        ...
        try:
            ...
            print(f"  → Loaded. Channels: {len(root.findall('channel'))}, Programmes: {len(root.findall('programme'))}")
            return root
        except Exception as e:
            ...
            if event == "end":
                if elem.tag == "channel":
                    ...
                    if cid and cid in valid_ids:
                        ...
                        # Jangan clear elem di sini karena dimasukkan ke list
                        continue 
                ...
                # Bersihkan elemen yang tidak terpakai dari memory root tree
                elem.clear()
                root.clear() # Mencegah leak memori di iterparse berkelanjutan
...
def merge_into_root(master_root: ET.Element, channels: list[ET.Element], programmes: list[ET.Element], seen_channel_ids: set[str]) -> None:
    for ch in channels:
        cid = ch.get("id")
...
def main() -> None:
    ...
    print(f"\n✅ Done. Channels: {len(master_root.findall('channel'))} | Programmes: {len(master_root.findall('programme'))}")
