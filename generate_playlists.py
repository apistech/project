...
def get_env_list(key: str, default: str = "all") -> tuple:
...
# --- Helper Functions ---
def fetch_url(
    url, is_json=True, is_gzipped=False, headers=None, stream=False, retries=3
):
    headers = headers or {"User-Agent": USER_AGENT}
    for i in range(retries):
        try:
            response = requests.get(
                url, headers=headers, timeout=REQUEST_TIMEOUT, stream=stream
            )
...
def format_extinf(
    channel_id, tvg_id, tvg_chno, tvg_name, tvg_logo, group_title, display_name
):
    chno_str = str(tvg_chno) if tvg_chno and str(tvg_chno).isdigit() else ""
    return (
        f'#EXTINF:-1 channel-id="{channel_id}" tvg-id="{tvg_id}" tvg-chno="{chno_str}" '
...
def generate_roku_m3u():
    ...
    for group in sorted(group_map.keys()):
        ...
        for c_id, ch in sorted(
            group_map[group],
            key=lambda x: (
                int(x[1].get("chno", 0))
                if str(x[1].get("chno", "")).isdigit()
                else 99999,
                x[1].get("name", "").lower(),
            ),
        ):
...
_TCL_DASH_RE = re.compile(
    r'^(.+?)\s+S(\d+)(?:\s+E(\d+))?(?:\s*[-–]\s*"?(.+?)"?\s*)?$', re.IGNORECASE
)
...
def parse_tcl_title(raw, api_season, api_episode):
    ...
    if m:
        return (
            ...
            int(m.group(2)),
            api_episode,
            _TCL_TRAILING_CODE.sub("", m.group(3)).strip() or None,
        )
...
def get_tcl_common_params():
    return {
        "userId": TCL_DEVICE_ID,
        "device_type": "web",
        "device_model": "web",
        "device_id": TCL_DEVICE_ID,
        "app_version": "1.0",
        "country_code": TCL_COUNTRY_CODE,
        "state_code": TCL_STATE_CODE,
    }
...
def resolve_tcl_stream(bundle_id, source, media):
    ...
    try:
        resp = session.post(
            f"{TCL_BASE_URL}/api/metadata/v1/format-stream-url",
            params=params,
            json=payload,
            timeout=20,
        )
        ...
        logger.debug(f"Failed to resolve stream for {bundle_id}: {e}")
        return media
...
def generate_tcl_m3u():
    ...
    try:
        livetab = session.get(
            f"{TCL_BASE_URL}/api/metadata/v2/livetab",
    ...
    for line in livetab.get("lines", []):
        ...
        try:
            data = session.get(
                f"{TCL_BASE_URL}/api/metadata/v1/epg/programlist/by/category",
                params=params,
                timeout=30,
            ).json()
            for ch in data.get("channels", []):
                ...
                if bid not in channels_map:
                    stream = resolve_tcl_stream(
                        bid, ch.get("source"), ch.get("media", "")
                    )
                    channels_map[bid] = {
                        "id": bid,
                        "name": ch.get("name"),
                        "logo": (
                            f"{TCL_IMAGE_BASE}{ch.get('logo_color')}"
                            if ch.get("logo_color")
                            else ""
                        ),
                        "stream": stream,
                        "category": cat_name,
        ...
        for i in range(0, len(unique_ids), batch_size):
            ...
            try:
                detail_resp = session.get(
                    f"{TCL_BASE_URL}/api/metadata/v1/epg/program/detail",
                    params=params,
                    timeout=30,
                ).json()
                details_list = (
                    ...
                    else [detail_resp] if isinstance(detail_resp, dict) else []
                )
    ...
    if TCL_GROUP_FILTER == "all":
        ...
        filtered_channels = [
            ch for ch in all_channels if ch["category"] in TCL_GROUP_FILTER
        ]

    sorted_channels = sorted(
        filtered_channels,
        key=lambda x: (x["category"].lower(), x["name"].lower()),
    )

    with open(
        os.path.join(OUTPUT_DIR, "tcl.m3u"), "w", encoding="utf-8"
    ) as f:
        f.write(f'#EXTM3U x-tvg-url="{TCL_EPG_URL}"\n')
        for ch in sorted_channels:
            f.write(
                f'#EXTINF:-1 tvg-id="{ch["id"]}" tvg-logo="{ch["logo"]}" group-title="{ch["category"]}",{ch["name"]}\n{ch["stream"]}\n'
            )
    ...
    for bid, p in stubs:
        ...
        prog_el = ET.SubElement(
            root, "programme", start=start_str, stop=stop_str, channel=bid
        )

        title = p.get("title", "No Title")
        clean_title, season, episode, subtitle = parse_tcl_title(
            title, p.get("season"), p.get("episode")
        )
        ET.SubElement(prog_el, "title").text = clean_title
        if subtitle or p.get("subtitle"):
            ET.SubElement(prog_el, "sub-title").text = (
                subtitle or p.get("subtitle")
            )
    ...
    ET.ElementTree(root).write(
        os.path.join(OUTPUT_DIR, "tcl_epg.xml"),
        encoding="utf-8",
...
def generate_pluto_m3u():
    ...
    if PLUTO_REGION_FILTER == "all":
        ...
        selected_regions = [
            r for r in PLUTO_REGION_FILTER if r in data["regions"]
        ]
    ...
    if PLUTO_GROUP_FILTER != "all":
        channels = {
            ...
            if v["service_group"] in PLUTO_GROUP_FILTER
        }

    sorted_channels = sorted(
        channels.items(),
        key=lambda x: (
            x[1]["service_group"].lower(),
            x[1].get("name", "").lower(),
        ),
    )
    ...
    output_lines = [
        f'#EXTM3U url-tvg="https://github.com/matthuisman/i.mjh.nz/raw/master/PlutoTV/{epg_region}.xml.gz"\n'
    ]
...
def generate_samsungtvplus_m3u():
    ...
    if SAMSUNG_REGION_FILTER == "all":
        ...
        selected_regions = [
            r for r in SAMSUNG_REGION_FILTER if r in data["regions"]
        ]
    ...
    if SAMSUNG_GROUP_FILTER != "all":
        channels = {
            k: v
            for k, v in channels.items()
            if v["service_group"] in SAMSUNG_GROUP_FILTER
        }
    ...
    output_lines = [
        f'#EXTM3U url-tvg="https://github.com/matthuisman/i.mjh.nz/raw/master/SamsungTVPlus/{epg_region}.xml.gz"\n'
    ]
